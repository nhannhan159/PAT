...
namespace PAT.Common.Classes.SemanticModels.DTMC
{
    public class DTMC
    {
        public Dictionary<string, DTMCState> States;
        public DTMCState InitState;

        private int Precision;
        private double MAX_DIFFERENCE;
        public List<DTMCState> TargetStates;
        //private List<DTMCState> TargetStates_backup = new List<DTMCState>();

        public DTMC(int precision, double maxdiffer)
        {
            Precision = precision;
        ...
        public override string ToString()
        {
            ...
            foreach (KeyValuePair<string, DTMCState> keyValuePair in States)
            {
        ...
        {
            foreach (KeyValuePair<string, DTMCState> state in States)
            {
            ...
            foreach (DTMCState targetState in TargetStates)
            {
        ...
        public double MaxProbability(VerificationOutput VerificationOutput, bool isAntiChain = false)
        {
        ...
        //public double DistributionProbCalc(DTMCState node, Distribution distr)
        //{
        //    double result = 0;
        ...
        //    {
        //        KeyValuePair<double, DTMCState> pair = distr.States[i];
        //        if (pair.Value == node)
        //        {
        ...
        //    else
        //    {
        //        foreach (KeyValuePair<double, DTMCState> pair in distr.States)
        //        {
    ...
    {
        public string ID;
        public List<KeyValuePair<double, DTMCState>> Transitions;
        public List<DTMCState> Pre;
        public double CurrentProb;
        public double CurrentReward;
        public HashSet<DTMCState> Sub;
        public bool AntichainUpdate = false;
        public bool ReachTarget = false;
        public int SCCIndex = -1;

        public DTMCState(string id)
        {
            ID = id;
            Transitions = new List<KeyValuePair<double, DTMCState>>();
            Pre = new List<DTMCState>();
            CurrentProb = 0;
            CurrentReward = 0;
            Sub = new HashSet<DTMCState>();
        }

        public void AddTransition(KeyValuePair<double, DTMCState> transition)
        {
        ...
        public void RemovePre()
        {
            ...
            {
                tran.Value.Pre.Remove(this);
            }
            //Transitions.Clear();
        }

        public void RemoveTransition(KeyValuePair<double, DTMCState> tran)
        {
            tran.Value.Pre.Remove(this);
        ...
        public override String ToString()
        {
            ...
            {
                toReturn += "[" + transition.Key + "]" + transition.Value.ID + "; ";
            }
